...
function generateRecommendations(smells: CodeSmell[]): string[] {
    ...
    const categories = smells.reduce((acc, smell) => {
        acc[smell.category] = (acc[smell.category] || 0) + 1;
        return acc;
    }, {} as Record<string, number>);

    if (categories["Complexity Issues"] > 10) {
        recommendations.push("🔴 HIGH PRIORITY: Consider breaking down complex functions into smaller, more focused functions.");
...
function generateHTMLReport(report: SmellReport): string {
    const categoryChartData = Object.entries(report.categories)
...
<body>
    ...
    <script>
        ...
        new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: [${Object.keys(report.categories).map(c => `"${c}"`).join(",")}],
                datasets: [{
...
function main(): void {
    ...
    console.log(`📊 Found ${report.totalSmells} issues in ${report.totalFiles} files`);
