...
## MANDATORY Final Quality Enforcement (ALWAYS before completing any task)

### 🚨 STRICT ERROR/WARNING & CONFIG ADHERENCE POLICY (MANDATORY)

- **Before starting any code-related task, you MUST read and understand the project's ESLint and Prettier configuration files.**
- **You MUST strictly follow ALL rules and conventions defined in ESLint and Prettier configs when creating, updating, or refactoring any file.**
- **You MUST NOT mark any task as complete if the output of `npm run lint:fix`, `npm run format`, or `npm run build` contains ANY errors or warnings.**
- **If there are ANY errors or warnings in the output, the task is NOT complete and you MUST fix all issues before proceeding.**
- **Only when the output of all quality commands is 100% clean (zero errors, zero warnings) may you consider the task complete.**
- **You MUST report the exact output of these commands before marking any task as complete.**
- **If any errors or warnings remain, you MUST continue fixing them until the output is fully clean.**
- **If any JS/TS/TSX file exceeds 300 lines, you MUST split it into multiple focused files before continuing.**
