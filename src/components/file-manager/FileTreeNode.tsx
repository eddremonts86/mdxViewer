...
import "@/assets/styles/FileTreeNode.css";
...
const useNodeState = (node: FileNode, expandedFolder: string | null, isSearching: boolean) => {
    const location = useLocation();
    const isActive = location.pathname === node.path;
    const hasActiveChild = node.type === "folder" && containsActiveFile(node, location.pathname);
    const isExpanded = node.type === "folder" && (isSearching || hasActiveChild || expandedFolder === node.path);
...
const NodeContent = ({
    ...
    <div className="flex min-w-0 flex-1 items-center gap-2">
        {icon}
        {node.type === "file" ? (
            <TransitionLink to={node.path} className="flex min-w-0 flex-1 items-center">
...
export function FileTreeNode({
    ...
    const { isActive, hasActiveChild, isExpanded } = useNodeState(node, expandedFolder, isSearching);
    ...
    return (
        <div>
            ...
            >
                {node.type === "folder" && (
                    <ToggleButton isExpanded={isExpanded} onToggle={handleToggle} isSearching={isSearching} />
                )}
                <NodeContent
                    node={node}
                    searchTerm={searchTerm}
                    isActive={isActive}
                    icon={<NodeIcon node={node} isExpanded={isExpanded} hasActiveChild={hasActiveChild} />}
                />
            </NodeButton>
